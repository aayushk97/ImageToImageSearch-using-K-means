...
public class ImageSearcher {
    ...
    public List<ImageResult> search(BufferedImage queryImage, double alpha, double beta, boolean useCluster) throws IOException {
        ...
        if (useCluster) {
            ...
            if (cid > -1) {
                ...
                clusterReader.close();
                System.out.println("Search: "+indexes.size());
            }
        } else {
            for (int c = 0; c < clusters.size(); c++) {
    ...
    private boolean roughFilter(double beta, double sigmaRQ, double sigmaGQ, double sigmaBQ,
        ...
        return ((beta * sigmaRQ) < sigmaRD) && (sigmaRD < (sigmaRQ / beta))
                && ((beta * sigmaGQ) < sigmaGD) && (sigmaGD < (sigmaGQ / beta))
                && ((beta * sigmaBQ) < sigmaBD) && (sigmaBD < (sigmaBQ / beta));
    }
